...
public class PlateauxModel {
    ...
    public PlayerModel getBiggestKnight() {
        return biggestKnight;
    }

    public PlayerModel getBiggestRoad() {
        return biggestRoad;
    }
                                ...
                                if(num == number){
                                    if (x + 1 <= getLength_x() - 1 && y + 1 <= getLength_y() - 1 && plateaux[y + 1][x + 1] instanceof Batiment && ((Batiment) plateaux[y + 1][x + 1]).getPlayer() != null) {
                                        ...
                                        if(plateaux[y + 1][x + 1].getName().contains("V")){
                                            ...
                                            }else if(viewModel instanceof GuiView){
                                                ((GuiView) viewModel).addLogs("Le joueur " + player.getColor() + " a obtenu " + 2 + " : " + res);
                                            }
                                        }else {
                                            ...
                                            if(viewModel instanceof TerminalView){
                                                ...
                                                ((GuiView) viewModel).addLogs("Le joueur " + player.getColor() + " a obtenu " + 1 + " : " + res);
                                            }
                                        }
                                        generate = true;
                                    }
    ...
    public Resources stealOneRessources(int x, int y, PlayerModel playerThief) {
        ...
        if(player != null && max != null){
            ...
            playerThief.setResources(max, playerThief.getResources().get(max)+1);
            if(viewModel instanceof GuiView){
                ((GuiView) viewModel).updateView();
            }
        }
    ...
    public void updateBiggestKnight(PlayerModel[] players){
        ...
        if(oldBiggestKnight != biggestKnight){
            if(viewModel instanceof TerminalView){
                ...
                ((GuiView) viewModel).addInfo("Le joueur " + biggestKnight + " a la plus grande armee");
            }
        }
    }
    ...
    public void updateLargestRoadPlayer(int x, int y, PlayerModel player, PlayerModel[] players) {
        ...
        if(oldBiggestRoad != biggestRoad){
            if(viewModel instanceof TerminalView){
                ...
                ((GuiView) viewModel).addInfo("Le joueur " + biggestRoad + " a la route la plus longue");
            }
        }
    }
